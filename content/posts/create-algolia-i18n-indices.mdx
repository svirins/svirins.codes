...
> I have omitted explicit type definitions for brevity
...
async function createIndexForLocale(indexName, locale) {
  try {
    ...
    console.log(
      `Saved ${data.length} documents to index: ${index.indexName}`,
    );
    return {
      status: 200,
      body: "Success!",
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const serializedError = e instanceof Error ? JSON.stringify(e,
